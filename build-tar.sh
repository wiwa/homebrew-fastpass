...
# $NATIVE -cp $CLASSPATH --initialize-at-build-time --no-server --no-fallback --allow-incomplete-classpath -H:ReflectionConfigurationFiles=$(pwd)/reflection.json scala.meta.internal.pantsbuild.BloopPants bin/fastpass
